...
## 10. Cohort v2 — the rebuild **[DONE — specified and coded, NOT YET RUN]**
...
## 11. The four gates **[BLOCKING — all four must pass]**
...
| item | blocker |
|---|---|
| **Fix B0** (PROPOSAL_FIXES open item A) | none — blocking, do it first |
| **Run the v2 rebuild** | pre-flight cleanup (§10.5); ~50 GB scratch; ~7.5 h Flye |
| Score the v2 cohort | **open item A must be fixed first** |
| B3 / B4 (GNN) | **the four gates in §11**, then the PyG dataset rebuild |
...
| Real cohorts (Wick / Johnson / NEKSUS) | deliberately deferred |
...
> Dataset acquisition and provenance validation are complete for the simulation
> and reference arms, and errors in the corpus description have been corrected
> against the deposits. The end-to-end read → assembly → labelling → feature →
> graph pipeline is implemented and validated on controlled simulated isolates
> with known ground truth, including a unit-tested labeller, a measured
> ligation/rapid prep effect, and zero assembler failures across 49 assemblies.
> The first full cohort was then rejected by its own adequacy gates — 12
> isolates is n=12, the small-plasmid band conflated a probabilistic failure
> mode with a deterministic one, and independently drawn plasmids share no
> sequence, so the two repeat-mediated label classes never materialised. A
> 50-isolate rebuild with recentred size bands and induced repeat structure is
> specified and coded; no model comparison will be reported until four numbered
> gates on that cohort pass. Large real-cohort transfers and assembly remain
> deferred until the pipeline passes its correctness gate.

The claim is **not** "the dataset is complete", and it is **not** "the baseline
ladder shows B1 > B0+". It is that the pipeline is validated, the cohort that
tested it was found inadequate by design criteria fixed in advance, and the
rebuild that follows is specified rather than improvised.
