...
CLASS_FLOORS = {"fragmented": 20, "absorbed": 20}
MIN_DISCORDANT = 20
...
def main():
    ...
    if all(v[2] for v in floor_status.values()):
        print("  All class floors met -- per-class claims are admissible.")

    # ----------------------------------------------------------------- 0b
    section("0b. REBUILD GATE PANEL (all four must pass)")
    d = discordance(rows)
    ...
    gates = [
        ...
        ("fragmented units >= %d" % CLASS_FLOORS["fragmented"],
         int_c.get("fragmented", 0) >= CLASS_FLOORS["fragmented"],
         "%d interventional fragmented units" % int_c.get("fragmented", 0)),
    ...
    print()
    if n_pass == len(gates):
        print("  ALL GATES PASS -- the cohort may proceed to B3/B4.")
    else:
    ...
    tp = r"D:\DNA-Sequencing\Dataset\assemblies.tar.gz"
